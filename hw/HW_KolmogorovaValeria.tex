...
\begin{center} \Large Колмогорова Валерия \end{center}
\bigskip
...
\begin{enumerate}
  ...
  \begin{myauto}
  	...
  	\path[->] 
  	(q_0) edge 				node [right] {$b$}    (q_3)
  	edge 					node [above] {$a$}    (q_1)
  	(q_1) edge 				node [right] {$b$}    (q_4)
  	edge 					node [above] {$a$}    (q_2)
  	(q_2) edge 				node [right] {$b$}    (q_5)
  	edge [loop right] 		node [right] {$a$}    ()
	(q_3) edge 				node [right] {$b$}    (q_6)
	edge 					node [above] {$a$}    (q_4)
	(q_4) edge 				node [right] {$b$}    (q_7)
	edge 					node [above] {$a$}    (q_5)
	(q_5) edge 				node [right] {$b$}    (q_8)
	edge [loop right] 		node [right] {$a$}    ()
	(q_6) edge 				node [above] {$a$}    (q_7)
	edge [loop below] 		node [below] {$b$}    ()
	(q_7) edge 				node [above] {$a$}    (q_8)
	edge [loop below] 		node [below] {$b$}    () 
	(q_8) edge [loop right] 		node [right] {$a, b$}    () 
  	;
