...
-- Добавил констрейнт Eq e
uberExpr :: (Eq e, Monoid e)
...
uberExpr ((op, Binary NoAssoc):opParsers) termP f g = parser <|> uberExpr opParsers termP f g
   where parser = do
...
uberExpr ((op, Unary):opParsers) termP f g = parser <|> uberExpr opParsers termP f g
    where parser = do
            operator <- op
            term <- uberExpr opParsers termP f g 
...
sepByOp :: (Eq e, Monoid e) => Parser e i op -> Parser e i ast -> Parser e i [(Maybe op, ast)]
sepByOp opP termP = parser <|> parser2
        where parser = do
                term <- termP
                op <- opP
