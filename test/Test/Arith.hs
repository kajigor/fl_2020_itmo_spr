...
import Arith (evaluate, parseNum, parseOp, parseSum, parseMult, parseExpr, Operator (..), AST (..))

unit_parseExpr :: Assertion
unit_parseExpr = do
    parseExpr "1+2+3" @?= Just (BinOp Plus (Num 1) (BinOp Plus (Num 2) (Num 3)), "")
    parseExpr "1+2*3" @?= Just (BinOp Plus (Num 1) (BinOp Mult (Num 2) (Num 3)), "")
    parseExpr "1/2*3" @?= Just (BinOp Mult (BinOp Div (Num 1) (Num 2)) (Num 3), "")
    parseExpr "1/2-3" @?= Just (BinOp Minus (BinOp Div (Num 1) (Num 2)) (Num 3), "")
    parseExpr "1-2+3" @?= Just (BinOp Minus (Num 1) (BinOp Plus (Num 2) (Num 3)), "")
