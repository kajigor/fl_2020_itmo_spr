...
parseExpr =  uberExpr [(parseOrOp, Binary RightAssoc),
           ...
           (Num <$> parseNum <|> symbol '(' *> parseExpr <* symbol ')' <|> parseFuncCall <|> Ident <$> parseIdent)
           BinOp
           UnaryOp
           where
...
parseAssign = Assign <$> parseId <*> (symbol '=' *> parseExp)
            where 
               ...
               parseExp = parseSeparators *> (parseExpr <|> parseFuncCall) <* parseSeparators
...
parseWrite = Write <$> (simpleParseKeyword "write" *> parseW) where
                parseW = do 
                      ...
                      left <- symbol '('
                      sep2 <- parseSeparators
                      result <- parseExpr <|> parseFuncCall
                      sep3 <- parseSeparators
                      right <- symbol ')'
                      sep4 <- parseSeparators
                      return $ result
