...
import           Expr             (evaluate, parseExpr, parseIdent, parseNum,
                                   parseOp)
...
unit_parseExpr = do
  ...
  runParser parseExpr "1*x+z*4" @?=
    Success "" (BinOp Plus (BinOp Mult (Num 1) (Ident "x")) (BinOp Mult (Ident "z") (Num 4)))
  runParser parseExpr "1+y*3+z" @?=
    Success "" (BinOp Plus (BinOp Plus (Num 1) (BinOp Mult (Ident "y") (Num 3))) (Ident "z"))
